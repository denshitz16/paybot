...
export default function AlipayPage() {
  ...
  const handleCreate = async () => {
    ...
    setResult(null);
    try {
      const res = await fetch('/api/v1/photonpay/alipay-session', {
        method: 'POST',
        headers: { 'Content-Type': 'application/json' },
        body: JSON.stringify({ amount: parseFloat(amount), description: description || 'Alipay payment' }),
      });
      ...
      if (!res.ok) {
        ...
        toast.success(data.message || 'Alipay session created!');
      } else {
    ...
    <Layout>
      <div className="max-w-5xl mx-auto">
        <div className="flex items-center gap-3 mb-6">
          ...
          <div>
            <h1 className="text-2xl font-bold text-foreground">Alipay</h1>
            <p className="text-sm text-muted-foreground">Create Alipay QR payment sessions via PhotonPay</p>
          </div>
        </div>
        ...
        <div className="grid grid-cols-1 lg:grid-cols-2 gap-6">
          <Card className="bg-card border-border">
            <CardHeader>
              <CardTitle className="text-foreground">Create Alipay Session</CardTitle>
            </CardHeader>
            <CardContent className="space-y-4">
              ...
              >
                {loading
                  ...
                  : <><Plus className="h-4 w-4 mr-2" />Create Alipay Session</>}
              </Button>
            </CardContent>
          </Card>
        ...
        <Card className="bg-card border-border mt-6">
          ...
          <CardContent>
            <code className="text-sm text-muted-foreground font-mono bg-muted px-3 py-2 rounded block">
              /alipay [amount] [description]
            </code>
            <p className="text-xs text-muted-foreground mt-2">
              Example: <span className="font-mono">/alipay 500 Order payment</span>
            </p>
          </CardContent>
        </Card>
      </div>
