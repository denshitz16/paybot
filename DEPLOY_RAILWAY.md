...
1. In your GitHub repository, go to Settings → Secrets → Actions and add the following secrets:
   ...
   - `RAILWAY_PROJECT_ID` — the Railway project id (you can find this in the Railway project settings or `railway project list --json`)

2. The repository already includes a workflow: `.github/workflows/deploy-railway.yml` which builds and deploys the app.

How the workflow works
- On `push` to `main` or when manually triggered, the workflow builds the Docker image, installs the Railway CLI, logs in, and runs `railway up` to deploy the project to the `production` environment.
...
railway up --project "$RAILWAY_PROJECT_ID" --environment production --detach
```
